...
export function ModalOffer({
    ...
    return (
        <Modal visible={offerOpen} transparent animationType="fade" onRequestClose={() => setOfferOpen(false)}>
            <Pressable onPress={() => setOfferOpen(false)} className="flex-1 bg-black/70 items-center justify-center p-[18px]">
                <Pressable onPress={() => { }} className="w-full max-w-[380px] rounded-[22px] p-4 bg-black/92 border border-white/12">
                    <Text className="text-white text-base font-black">{selectedBar?.name ?? "Offres"}</Text>
                    <Text className="text-white/70 mt-1.5">Choisis une offre pour générer le QR code</Text>

                    <View className="mt-3.5 gap-2.5">
                        {(selectedBar?.offers ?? []).map((offer) => {
                            const alreadyHas = qrcodes?.some((qr) => qr.barId === selectedBar!.id && qr.productId === offer.id);
                            return (
                                <Pressable
                                    key={offer.id}
                                    onPress={() => onCreateQrcode({
                                    ...
                                    className={clsx(
                                        ...
                                        isCreateQrcodePending && "opacity-65"
                                    )}
                                >
                                    ...
                                    {typeof offer.price === "number" && <Text className="text-white/70 mt-1">{formatPrice(offer.price)}</Text>}
                                </Pressable>
                            );
                        })}
                        {(!selectedBar?.offers || selectedBar.offers.length === 0) && <Text className="text-white/70">Aucune offre disponible.</Text>}
                    </View>
                    ...
                    <View className="mt-3.5 flex-row items-center">
                        {(isCreateQrcodePending || isGetMyQrcodesPending) && (
                            <>
                                <ActivityIndicator />
                                <Text className="text-white/75 ml-2">{isCreateQrcodePending ? "Génération…" : "Chargement du stock…"}</Text>
                            </>
                        )}
                    </View>

                    <Pressable onPress={() => setOfferOpen(false)} className="mt-3.5 py-3 rounded-[14px] bg-white/10 items-center">
